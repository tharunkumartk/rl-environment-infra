...
class BrowserAgent:
    ...
    def get_model_response(
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Logger will handle error tracking, no need to print here
    ...
    def run_one_iteration(self) -> Literal["COMPLETE", "CONTINUE"]:
        ...
        if self._verbose:
            with console.status(
                ...
                try:
                    ...
                    error_msg = f"Model generation failed: {type(e).__name__}: {str(e)}"
                    if self.logger:
                        import traceback
                        ...
                        self.logger.log_error(
                            ...
                            context={"phase": "model_generation"},
                        )
                    return "COMPLETE"
        else:
            try:
        ...
        if not response.candidates:
            if self.logger:
                self.logger.log_error(
                    error_message="Empty response - no candidates",
        ...
        if not function_calls:
            # Agent loop complete - no more function calls
            self.final_reasoning = reasoning
            if self._verbose:
                print(
                    f"[DEBUG] Agent completed with no function calls. Reasoning: {reasoning[:200] if reasoning else 'None'}"
                )
            return "COMPLETE"

        # Prepare function call data for logging
        function_call_data = []
        for function_call in function_calls:
            ...
            function_call_data.append(fc_dict)
        ...
        # Verbose output disabled when running in suppressed mode
        ...
        for function_call in function_calls:
            ...
            if function_call.args and (
                ...
                if decision == "TERMINATE":
                    # Safety decision: terminate agent loop
                    return "COMPLETE"
            ...
            if isinstance(fc_result, EnvState):
                # Capture the last screenshot for logging
                last_screenshot = fc_result.screenshot
        ...
        # Log this step if logger is available
        if self.logger:
            self.logger.log_step(
                reasoning=reasoning,
                function_calls=function_call_data,
    ...
    def _get_safety_confirmation(
        ...
        # Auto-accept safety confirmations (no output needed when suppressed)
        return "CONTINUE"
