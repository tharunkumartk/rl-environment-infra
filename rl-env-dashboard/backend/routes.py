...
async def get_task(task_id: str):
    ...
    task["completed_count"] = sum(1 for r in rollouts if r["status"] in ["success", "failed"])
    task["success_count"] = sum(1 for r in rollouts if r["status"] == "success")
...
async def get_job(job_id: str):
    ...
    job["completed_count"] = sum(1 for r in rollouts if r["status"] in ["success", "failed"])
    job["success_count"] = sum(1 for r in rollouts if r["status"] == "success")
    ...
    for rollout in rollouts:
        if rollout["status"] in ["pending", "provisioning", "running"]:
            try:
                ...
                logger.error(f"Error tearing down environment for rollout {rollout['id']}: {e}")
...
async def delete_rollout(rollout_id: str):
    ...
    if rollout["status"] in ["pending", "provisioning", "running"]:
        try:
            ...
            logger.error(f"Error tearing down environment: {e}")
...
async def get_compute_stats(
    ...
    completed = sum(1 for r in all_rollouts if r["status"] in ["success", "failed"])
    failed = sum(1 for r in all_rollouts if r["status"] in ["failed", "error"])
    success_count = sum(1 for r in all_rollouts if r["status"] == "success")
    ...
    # Get recent completed and failed rollouts (last 20)
    completed_rollouts = [r for r in all_rollouts if r["status"] in ["success", "failed"]]
    completed_rollouts.sort(
        key=lambda x: x.get("completed_at") or x.get("created_at"), reverse=True
    )
    recent_completed = completed_rollouts[:20]

    failed_rollouts = [r for r in all_rollouts if r["status"] in ["failed", "error"]]
    failed_rollouts.sort(
        key=lambda x: x.get("completed_at") or x.get("created_at"), reverse=True
    )
    ...
    return {
        ...
        "recent_completed_rollouts": recent_completed,
